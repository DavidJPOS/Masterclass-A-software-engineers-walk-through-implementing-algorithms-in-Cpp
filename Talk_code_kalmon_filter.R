...
for(row_i in 1:(nrow(res) - 1)){ # row_i <- 1
  
  # Iterate the following
  prediction <- kalman_filter$predict()
  z <- array(c(res$x_mes[row_i + 1], res$y_mes[row_i + 1]), c(2, 1)) 
  estimate <- kalman_filter$update(z)
  
  res$x_est[row_i + 1] <- estimate$posterior[1]
  res$y_est[row_i + 1] <- estimate$posterior[2]
  
  res$x_residual[row_i + 1] <- estimate$residual[1]
  res$y_residual[row_i + 1] <- estimate$residual[2]
  
  res$x_sd[row_i + 1] <- sqrt(estimate$posterior_covariance[1,1])
  res$y_sd[row_i + 1] <- sqrt(estimate$posterior_covariance[2,2])
}

res <- res %>% 
  mutate(
  x_true_residual = x_true - x_est,
  y_true_residual = y_true - y_est
...
ggsave(filename = './1_xy_overtime.png', plot = pg1)


# create the residuals plots
p4 <- ggplot(res, aes(x = time, y = x_true_residual)) + 
...
p5 <- ggplot(res, aes(x = x_true_residual)) +
...
p6 <- ggplot(res, aes(x = time, y = y_true_residual)) + 
...
p7 <- ggplot(res, aes(x = x_true_residual)) +
  geom_histogram(color = 'black', fill = 'steelblue') +
  xlim(-2,2) + 
  xlab('y residual')  
...
ggsave(filename = './2_filter_residuals.png', plot = pg2, width = 9, height = 6)
# animation -----------------------------------------------------------------------------------
